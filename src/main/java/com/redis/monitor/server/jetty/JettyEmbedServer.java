...
public class JettyEmbedServer {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(JettyEmbedServer.class);
	...
	public void start() throws Exception {
		...
		if (handler != null && handler instanceof WebAppContext) {
			...
			webAppContext.setResourceBase(JettyEmbedServer.class.getResource(
					"/webapp").toString());
		}
		server.start();
	...
	public static void main(String[] args) throws Throwable {
		try {
			final JettyEmbedServer server = new JettyEmbedServer("jetty.xml");
			server.start();
			...
			Runtime.getRuntime().addShutdownHook(new Thread() {
				...
				public void run() {
					try {
						...
						logger.warn(
								"##something goes wrong when stopping manager Server:\n{}",
			...
			logger.error(
					"## Something goes wrong when starting up the manager Server:\n{}",
					ExceptionUtils.getFullStackTrace(e));
			System.exit(0);
		}
	}
